(ns bench-hnswlib-java
  "Benchmark hnswlib-java (jelmerk) on SIFT dataset.

   Usage:
     clj -M:benchmark -m bench-hnswlib-java sift10k
     clj -M:benchmark -m bench-hnswlib-java sift10k 16 200 100

   Outputs JSON results to stdout."
  (:require [clojure.data.json :as json]
            [clojure.set :as set])
  (:import [com.github.jelmerk.hnswlib.core.hnsw HnswIndex]
           [com.github.jelmerk.hnswlib.core DistanceFunctions Item SearchResult]
...
;; -----------------------------------------------------------------------------
;; hnswlib-java Item wrapper

(deftype VectorItem [^int id ^floats vector]
  Item
  (id [_] (Integer/valueOf id))
  (vector [_] vector)
  (dimensions [_] (alength vector)))

;; -----------------------------------------------------------------------------
;; hnswlib-java benchmark
...
(defn run-benchmark [{:keys [dataset M ef-construction ef-search k]}]
  (let [use-cosine? (glove-dataset? dataset)
    ...
    ;; Build index
    (println "\nBuilding index...")
    (let [index (-> (HnswIndex/newBuilder dim distance-fn n-vectors)
                    (.withM M)
                    (.withEfConstruction ef-construction)
                    (.withEf ef-search)
                    (.build))

          ;; Create items
          items (map-indexed (fn [i v] (->VectorItem i v)) vectors)

          start (System/nanoTime)
          _ (.addAll index items)
      ...
      (println "\nWarmup...")
      (dotimes [_ 10]
        (.findNearest index (first query-vecs) k))
      ...
      (let [search-results (atom [])
            latencies (doall
                        (for [i (range n-queries)]
                          (let [q (nth query-vecs i)
                                start (System/nanoTime)
                                results (.findNearest index q k)
                                elapsed (- (System/nanoTime) start)
                                result-ids (map #(.item %) results)]
                            (swap! search-results conj (vec result-ids))
                            elapsed)))
        ...
        (let [recalls (for [i (range n-queries)]
                        (let [result-ids (map #(.id %) (nth @search-results i))
